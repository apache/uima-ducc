...
var wip_reliable = false;

function ducc_reliable() {
    var fname = "ducc_reliable";
    var data = null;
    if(wip_reliable) {
        ducc_console_warn(fname+" already in progress...")
        return;
    ...
    wip_reliable = true;
    try {
        var servlet = "/ducc-servlet/cluster-reliable-label";
        var tomsecs = ms_timeout;
        $.ajax({
            url: servlet,
            timeout: tomsecs
    ...
    wip_reliable = true;
    try {
        var servlet = "/ducc-servlet/cluster-reliable-status";
        var tomsecs = ms_timeout;
        $.ajax({
            url: servlet,
            timeout: tomsecs
...
function ducc_submit_login() {
    ...
    try {
        ...
        var userid = textify(document.forms[1].userid.value)
        ducc_update_uid()
...
function ducc_submit_reservation() {
    ...
    try {
        ...
        var scheduling_class = textify(e.options[e.selectedIndex].value);
        var e = document.getElementById("memory_size");
        var memory_size = textify(e.value);
        var e = document.getElementById("memory_units");
        var memory_units = textify(e.options[e.selectedIndex].value);
        var e = document.getElementById("description");
        var description = textify(e.value);
        var e = document.getElementById("wait_for_result_yes");
        var wait_for_result = textify(e.checked);
        if (wait_for_result) {
...
function ducc_update_service(name) {
    ...
    try {
        ...
        var autostart = textify(e.options[e.selectedIndex].value);
        var e = document.getElementById("instances");
        var instances = textify(e.value);
