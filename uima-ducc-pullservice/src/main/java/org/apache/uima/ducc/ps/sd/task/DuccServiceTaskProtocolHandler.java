...
public class DuccServiceTaskProtocolHandler implements TaskProtocolHandler {
    ...
    try {
      ...
      switch (type) {
        case Get:
          if (logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE,
                    "---- Driver handling GET Request - Requestor:" + taskConsumer.toString());
          }
        ...
        case Ack:
          if (logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE,
                    "---- Driver handling ACK Request - Requestor:" + taskConsumer.toString());
          }
        ...
        case End:
          if (logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE,
                    "---- Driver handling END Request - Requestor:" + taskConsumer.toString());
          }
  ...
  private IMetaMetaTask getMetaMetaTask(TaskConsumer taskConsumer) {
    ...
    if (task != null && !task.isEmpty()) {
      ...
      if (logger.isLoggable(Level.FINE)) {
        logger.log(Level.FINE,
                "Returning TASK with appdata:" + task.getMetadata() + " to the service");
      }
  ...
  private void handleMetaTaskTransationEnd(IMetaTaskTransaction trans, TaskConsumer taskConsumer) {
    ...
    if (trans.getMetaTask().getUserSpaceException() != null) {
      ...
      taskAllocator.onTaskFailure(taskConsumer, trans.getMetaTask().getAppData(),
              exceptionAsString);

    } else {
      String m = trans.getMetaTask().getPerformanceMetrics();
      if (logger.isLoggable(Level.FINE)) {
        logger.log(Level.FINE, "handleMetaTaskTransationEnd()........... appdata:"
