...
public class Utils {
  ...
  /**
   * Create an array of parameters from a whitespace-delimited list (e.g. JVM args or environment
   * assignments.) Values containing whitespace must be single- or double-quoted: TERM=xterm
   * DISPLAY=:1.0 LD_LIBRARY_PATH="/my/path/with blanks/" EMPTY= -Dxyz="a b c" -Dabc='x y z' Quotes
   * may be stripped or preserved. Values containing both types of quotes are NOT supported.
   *
   * @param options
    ...
    if (stripQuotes) {
      while (matcher.find()) {
        ...
        if (matcher.end() >= options.length()
                || Character.isWhitespace(options.charAt(matcher.end()))) {
  ...
  /**
   * Resolves placeholders in provided contents using java's Matcher. Finds all occurances of
   * ${<placeholder>} and resolves each using System properties which holds <placeholder>=<value>
   * pairs.
   * 
   * @param contents
  ...
  /**
   * Resolves placeholders in provided contents using java's Matcher. Finds all occurances of
   * ${<placeholder>} and resolves each using provided Properties object which holds
   * <placeholder>=<value> pairs. If the placeholder not found then tries the System properties.
   * 
   * @param contents
   *          - target text containing placeholder(s)
   * @param props
