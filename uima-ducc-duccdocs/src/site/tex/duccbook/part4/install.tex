...
\begin{itemize}
  ...
  \item Reasonably current Linux.  DUCC has been tested on SLES 10.2, 11.1, and 11.2, and RHEL 6.3.
    ...
    {\em Note:} On some systems the default {\em user limits} are defined too low for DUCC.  The shell
    start-up scripts for user {\em ducc} should set the soft limit to be the same as the hard limit.  
    During DUCC boot, the DUCC start-up scripts will emit a warning if the limits are too low.
