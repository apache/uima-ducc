...
DuccTextCM finds 768 paragraphs in moby.txt. The total size of the resultant 768 XmiCas files is 108MB. The total size using ZIP compression and packaging these into 13 output Work Item zipfiles is 20MB. Using UIMA binary compressed format the further reduces total size to 6.5MB.
...
The average uncompressed size of XmiCas formatted CASes from processing the 768 paragraphs in moby.txt is 140KB.
Using UIMA binary compression format for both input and output CASes, a single-threaded Jop Process will complete
this CAS processing task in about 8 seconds. That translates to roughly 100 CASes per second: reading, deserializing, 
re-serializing and re-writing the CAS data.

Although this thread is primarily CPU bound doing serialization work, it is possible to become I/O bound 
with enough threads banging on a shared filesystem. For this sample job doing no additional processing, 
DuccCasInputSpec.job limits the total number of processing threads to 32 using the combination 
