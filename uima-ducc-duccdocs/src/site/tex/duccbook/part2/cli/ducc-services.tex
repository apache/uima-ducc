...
\end{verbatim}
         ...
         regardless of the number specified in the registration. The registry is updatedo if the
...
\end{verbatim}
       ...
       \item[--instances {[integer]}] This is the number of instances to stop. If omitted, all
         ...
         number of instances are stopped. Thus if ten instances are running for a service
...
\end{verbatim}
         ...
         specified. The registered number of instances is never reduced to 0 even if the number of
...
\end{verbatim}
    ...
    \item[--query {[service-id or endpoint]}] This indicates that a service is to be stopped. The
      ...
      submitted outside of DUCC so it is marked as Implicit and has no implementing processes 
