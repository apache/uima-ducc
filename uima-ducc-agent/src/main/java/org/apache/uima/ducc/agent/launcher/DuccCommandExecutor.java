...
public class DuccCommandExecutor extends CommandExecutor {
	...
	private void stopProcess(ICommandLine cmdLine, String[] cmd)
		...
		try {
			...
			if (((ManagedProcess) managedProcess).doKill()
					...
					|| ((ManagedProcess) managedProcess).getDuccProcess()
							.getProcessState().equals(ProcessState.Starting)
					|| ((ManagedProcess) managedProcess).getDuccProcess()
							.getProcessState()
				...
				if (((ManagedProcess) managedProcess).getDuccProcess()
					...
					// String[] sigTermCmdLine = new String[]
					// {"/bin/kill","-15",((ManagedProcess)
				    ...
				    try {
						logger.info(methodName,
								((ManagedProcess) super.managedProcess)
										.getDuccId(),
								"------------ Agent Starting Timer For Process with PID:"
										+ ((ManagedProcess) managedProcess)
												.getDuccProcess().getPID()
										+ " Process State: "
										+ ((ManagedProcess) managedProcess)
				...
				// Add PID to the header. Receiving process has a PID filter in
				// its router
				// to determine if the stop message is destined for it.
				msgHeader.put(DuccExchange.ProcessPID,
						((ManagedProcess) managedProcess).getPid());
				msgHeader.put(DuccExchange.DUCCNODEIP,
						((ManagedProcess) managedProcess).getDuccProcess()
								.getNodeIdentity().getIp());
				logger.info(methodName,
						((ManagedProcess) super.managedProcess).getDuccId(),
						"Agent Sending Stop request to remote process with PID:"
								+ ((ManagedProcess) managedProcess).getPid()
								+ " On Node:"
								+ ((ManagedProcess) managedProcess)
										.getDuccProcess().getNodeIdentity()
										.getIp());
				try {
					// Use either Mina (socket) or jms to communicate with an
					// agent
					if (((ManagedProcess) managedProcess).getSocketEndpoint() != null) {
						...
						logger.info(
								methodName,
								null,
								"Agent Sending <<STOP>> to Endpoint:"
										+ ((ManagedProcess) managedProcess)
												.getSocketEndpoint());
						// socket transport
						super.agent
								...
								.dispatch(
										new ProcessStopDuccEvent(
												new HashMap<DuccId, IDuccProcess>()),
										((ManagedProcess) managedProcess)
												.getSocketEndpoint(), msgHeader);
					...
					logger.info(
							methodName,
							((ManagedProcess) super.managedProcess).getDuccId(),
							"------------ Agent Dispatched STOP Request to Process with PID:"
									+ ((ManagedProcess) managedProcess)
											.getDuccProcess().getPID()
									+ " Process State: "
									+ ((ManagedProcess) managedProcess)
											.getDuccProcess().getProcessState()
									+ " Waiting for Process to Stop. Timout Value:"
					...
					try {
						...
						if (((ManagedProcess) super.managedProcess)
								.getDuccProcess().getProcessState()
								.equals(ProcessState.Running)) {
							// the following call will block!!! Waits for
							// process to stop on its own. If it doesnt stop,
							// kills it hard.
							// The exact time the service is allotted for
							// stopping is defined in ducc.properties
							logger.info(methodName,
									((ManagedProcess) super.managedProcess)
											.getDuccId(),
									"------------ Agent Starting Timer For Process with PID:"
					...
					} catch (TimeoutException tex) { // on time out kill the
														// process
						if (((ManagedProcess) super.managedProcess)
								.getDuccProcess().getProcessState()
								.equals(ProcessState.Running)
							...
							logger.info(
									...
									"------------ Agent Timed-out Waiting for Process with PID:"
											+ ((ManagedProcess) managedProcess)
													.getDuccProcess().getPID()
											+ " to Stop but the process is not in a running state. Process State:"
											+ ((ManagedProcess) managedProcess)
													.getDuccProcess()
													.getProcessState());
						}
	...
	private void doExec(ProcessBuilder pb, String[] cmd, boolean isKillCmd)
		...
		try {
			...
			} else if (!((ManagedProcess) managedProcess).getDuccProcess()
					.getProcessState()
